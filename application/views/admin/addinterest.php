...
<script>
	...
	/*$("#formInterest").validate({
		...
		messages: {
		  ...
		  $('#interestopt').click(function(){
		 		//alert('aaaaaaa'); 
				$('#subinteresttxt').hide();
				$('#subinterest').hide();
				$('#interesttxt').show();
		  })
		  $('#subinterestopt').click(function(){
		 		//alert($('#opt_interest_id').val()); 
				if($('#opt_interest_id').val()!=0){
...
function openDiv(val){
			//alert(val);
			if(val!=0){
				$("#interest_name").attr("disabled", "disabled"); 
				$('#flag').val(2);
				$('#subinterest').show();
			}else{
				$("#interest_name").removeAttr("disabled");
				$('#flag').val(1); 
				$('#subinterest').hide();
			}
			//alert('aaa');
...
function validate(){
		...
		if($("#subinteresttxt").is(":visible")){
		
			if($('#opt_interest_id').val()=='')
				{
						
						alert('Please select Interest Name  !!');
						//$('#interest_name').focus();
						return false;
				}
				if($('#subinterest_name').val()=='')
				{
						
						alert('Please enter SubInterest Name  !!');
						$('#subinterest_name').focus();
						return false;
				}
		}
}
				...
				<div class="col-md-12 ">
					...
					<div class="portlet">
						<div class="portlet-title">
							<div class="caption">Add Interest
						...
						<div class="portlet-body form">
							       <form name="formInterest" class="form-horizontal" id="formInterest" method="post" action="<?php  echo base_url('admin/interest/addinterest'); ?>"  onsubmit="return validate()" >
								   	<div class="form-body">
									<div class="form-group">
                                            <label class="col-md-2 control-label" for="exampleInputEmail1">Select Type</label>
                                           	 	<div class="col-md-4">
													<div class="radio-list">
														<label class="radio-inline">
															<input type="radio" name="status" value="0" id="interestopt" checked="checked">&nbsp;Interest
														</label>
														<label class="radio-inline">
															<input type="radio" name="status" value="1"  id="subinterestopt">&nbsp;Subinterest
														</label>
											   		</div>
												</div>
                                            
                                      </div>
									<div class="form-group"  id="interesttxt">
                                            <label class="col-md-2 control-label" for="exampleInputEmail1">Add Interest</label>
                                            <div class="col-md-4">
											<input type="text" name="interest_name" id="interest_name"  class="form-control"  value=""/>
                                            </div>
                                       </div>
									  <div class="form-group" style="display:none;" id="subinteresttxt">
                                            <label class="col-md-2 control-label" for="exampleInputEmail1">Select Interest</label>
                                            <div class="col-md-4">
											<select  name="opt_interest_id" id="opt_interest_id"  class="form-control" onchange="openDiv(this.value)">
											<option value="">--Select Interest--</option>
												<?php foreach($getallinterests as $key=>$val){ ?>
												  <option value="<?php echo $val['id']; ?>"><?php echo $val['interest_name']; ?></option>
												<?php } ?>
											</select>
											<!--<input type="text" name="subinterest_name" id="subinterest_name"  class="form-control"  value=""/>-->
                                            </div>
                                       </div>
									   <div class="form-group" style="display:none;" id="subinterest">
                                            <label class="col-md-2 control-label" for="exampleInputEmail1">Add SubInterest</label>
                                            <div class="col-md-4">
											<input type="text" name="subinterest_name" id="subinterest_name"  class="form-control"  value=""/>
                                            </div>
                                       </div>
