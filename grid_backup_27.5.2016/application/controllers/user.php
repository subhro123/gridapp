					 ...
					 if($check_user_login ==0){
					        ...
					        if(isset($userData['email'])){
								...
								if(isset($user_id['id'])){
									...
									$this->db->where('id',$user_id['id'])->update('user',array('last_login' => $last_login));
					...
					if(!empty($contacts)){
						...
						foreach($getalluserimportbyuser as $x=>$importval){
									...
									if(!empty($datanew[$x])){
										...
										$datas[$x]['user_id']=($result['id']!='')?$result['id']:NULL;
			...
			if(isset($userData['is_featured']) && $userData['is_featured']=='1'){
				...
				$unique_user_id = $this->Muser->create('paymentlog',$datapayment);
...

							{
								...
								$this->Response->outputResponse(true, false, array("response" =>  '' ), '', '', 'Event created successfully!!');	
	...
	public function user_position_post()

		{
			   ...
			   if(isset($userData['post_lat'])){
				...
				$chkuserpositionbyuserid=$this->Muser->chkUserPositionByUserid($user_id);
					...
					$result = $this->functions->update('user_position',$data,$user_id,'user_id');
