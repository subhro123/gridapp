...
gridApp.controller('ChatCtrl',function($scope, $ionicSideMenuDelegate,$ionicHistory,$stateParams,socket,$sanitize,$ionicScrollDelegate,$timeout,ControVarAccessFac,$filter,$cordovaCamera,$cordovaDevice,$cordovaFile,$ionicActionSheet,$cordovaFileTransfer,$cordovaImagePicker,$rootScope) {	
			...
			$scope.getUserInfo=function(user_id){
				...
				console.log("XXXXX"+JSON.stringify(user[0]))
				return user[0];
			}
			
			
			$scope.ChatUser.userId=String(ControVarAccessFac.getData("user_ID"));
			$scope.ChatUser.name=ControVarAccessFac.getData("userName");
			$scope.ChatUser.pro_image=ControVarAccessFac.getData("userImage");
			
			//$scope.socket.emit("join", ControVarAccessFac.getData("user_ID"));
			$scope.socket.emit("join", $scope.ChatUser);
			
			$scope.socket.on("personalchat", function(who, msg,type){
					console.log("personalchat  who::"+who+" >>>> "+msg)
					$scope.messages.push({content:$sanitize(msg),username:who.name,image:who.pro_image,type:type})
					//if (!$scope.$$phase) $scope.$apply();
  					$ionicScrollDelegate.scrollBottom();
			});
						 
			$scope.socket.on("personalchatSelf", function(who, msg,type){
			...
			$scope.sendMessage=function(){				
				selectedUserID=String(ControVarAccessFac.getData("other_user_ID"));
				console.log("selectedUserID::: "+selectedUserID)
				type="text"
				$scope.socket.emit("send", $scope.message,selectedUserID,type);
				$scope.message='';
			}
		...
		document.addEventListener("deviceready", function () {
		 ...
		 $rootScope.$on('event:file:selected',function(event,data){
			 ...
			 // var imageUrl=data.replace(/data:image\/jpeg;base64,/g, '');
			 	var imageUrl=data
				var type="media"
			  $scope.socket.emit("send", imageUrl,String(ControVarAccessFac.getData("other_user_ID")),type);
