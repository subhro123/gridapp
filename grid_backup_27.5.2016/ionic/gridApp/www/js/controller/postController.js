			...
			$scope.changePostView=function(){
				...
				for(var j=0;j<$scope.post.mapViewMarkerIconArr.length;j++){
									...
									/*var contentString = '<div id="content" ng-click="openPostDetailsView("'+$scope.postResponseFilter[j].id+','+$scope.postResponseFilter[j].user_id+')">'+
									  ...
									  var contentString = '<div id="content" ng-click="openPostDetailsView('+$scope.postResponseFilter[j].id+','+String($scope.postResponseFilter[j].user_token)+','+$scope.postResponseFilter[j].event_date_timestamp+')">'+
									  '<div id="siteNotice">'+
									  '</div>'+
									  '<h5 id="firstHeading" class="firstHeading">'+$scope.postResponseFilter[j].post_title+'</h5>'+
									  '<div id="bodyContent">'+
				...
				/*==================postdetailviewopen ======*/
					...
					$scope.openPostDetailsView= function(post_id,user_token,event_date_timestamp){
  			...
  			console.log("The date entered differs from today's date by " + difference + " milliseconds");	
					...
					if(difference>0){
						...
						if(user_token==ControVarAccessFac.getData("user_ID")){
							console.log("owner...")
